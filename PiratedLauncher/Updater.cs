...
namespace PiratedLauncher
{
    public partial class Updater : Form
    {
        ...
        private async void Updater_Load(object sender, EventArgs e)
        {
            ...
            // we'll need a local backend now that piratedlauncher is dead XD
            string JsonResponse;
            string ApiVersion;
            ApiVersion = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Backend", "apiVersion.json");
            JsonResponse = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Backend", "jsonResponse.json");
            // end of local backend
            ...
            try
            {
                apiVersion = File.ReadAllText(ApiVersion);
                jsonResponse = File.ReadAllText(JsonResponse);
            }
            catch (Exception ex)
            {
        ...
        private async Task<Panel> CreateGamePanelAsync(Game game)
        {
            ...
            try
            {
                pictureBox.Image = LoadImageFromPath(game.Name);
            }
            catch
            {
        ...
        private Image LoadImageFromPath(string gameName)
        {
            try
            {
                string imagePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Backend", $"{gameName}.png");
